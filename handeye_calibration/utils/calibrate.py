...
from utils import axyb
import numpy as np
import pudb


def axxbMethod(A, B, lm_tol, ex_method):
    """axxbMethod.
...
def axybMethod(A, B, lm_tol, ex_method):
    ...
    for method in axyb_method:
        if method == 10:
            ...
            X_Tabb_zc1, Y_Tabb_zc1, time_Tabb_zc1 = axyb.tabb_zc1(A, B, X0, Y0, lm_tol)
            X = np.concatenate((X, np.expand_dims(X_Tabb_zc1, axis=0)), axis=0)
            Y = np.concatenate((Y, np.expand_dims(Y_Tabb_zc1, axis=0)), axis=0)
        elif method == 13:
            X0, Y0 = X_Li, Y_Li # Initial matrix
            X_Tabb_zc2, Y_Tabb_zc2, time_Tabb_zc2 = axyb.tabb_zc2(A, B, X0, Y0, lm_tol)
